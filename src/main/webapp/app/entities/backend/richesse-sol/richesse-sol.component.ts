...
export class RichesseSolComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.richesseSolService
            ...
            .subscribe(
                (res: HttpResponse<IRichesseSol[]>) => this.paginateRichesseSols(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/richesse-sol'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    protected paginateRichesseSols(data: IRichesseSol[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.richesseSols = data;
    }
