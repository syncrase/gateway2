...
export class FloraisonComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.floraisonService
            ...
            .subscribe(
                (res: HttpResponse<IFloraison[]>) => this.paginateFloraisons(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/floraison'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    protected paginateFloraisons(data: IFloraison[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.floraisons = data;
    }
